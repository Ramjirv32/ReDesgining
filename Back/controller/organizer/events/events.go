...
func EventsLogin(c *fiber.Ctx) error {
	...
	if err := organizersvc.SendOTP(req.Email, "events"); err != nil {
...
func VerifyOTP(c *fiber.Ctx) error {
	...
	if org.CategoryStatus == nil || org.CategoryStatus["events"] != "approved" {
		_ = organizersvc.UpdateCategoryStatus(org.ID.Hex(), "events", "approved")
		if org.CategoryStatus == nil {
			org.CategoryStatus = map[string]string{}
		}
		org.CategoryStatus["events"] = "approved"
	}
...
func SubmitVerification(c *fiber.Ctx) error {
	...
	var v models.EventVerification
	if err := c.BodyParser(&v); err != nil {
		return c.Status(400).JSON(fiber.Map{"error": err.Error()})
	}
	v.OrganizerID = orgObjID
	if err := verifysvc.SubmitEventVerification(&v); err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
